...
ds = as.numeric(time(UKDriverDeaths)) # making the time series numeric.
y = as.numeric(UKDriverDeaths) # making the time series numeric.

lm_model<- lm(ds~y, data = UKDriverDeaths) # using lm function for the numeric columns into a linearly model.
summary(lm_model) # using summary function to output the simple explanation of our linear model.
